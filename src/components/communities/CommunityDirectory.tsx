...
const text = {
  en: {
    eyebrow: "NOXA COMMUNITIES", title: "Find your scene.", body: "Discover automotive and motorcycle communities, organisers and local scenes across Greece. One public directory, connected to real events.",
    searchLabel: "Search communities", searchPlaceholder: "Name, city or scene", all: "All", car: "Car", moto: "Moto", mixed: "Mixed", found: "communities", view: "View community",
    emptyTitle: "The directory is opening now.", emptyBody: "NOXA only lists real communities. As communities are reviewed and published, they will appear here instead of using placeholder profiles or fake numbers.", emptyCta: "List your community", footer: "Public community discovery by NOXA.",
  },
  el: {
    eyebrow: "NOXA COMMUNITIES", title: "Βρες τη σκηνή σου.", body: "Ανακάλυψε automotive και moto κοινότητες, διοργανωτές και local scenes σε όλη την Ελλάδα. Ένας δημόσιος κατάλογος συνδεδεμένος με πραγματικά events.",
    searchLabel: "Αναζήτηση κοινοτήτων", searchPlaceholder: "Όνομα, πόλη ή scene", all: "Όλα", car: "Car", moto: "Moto", mixed: "Mixed", found: "κοινότητες", view: "Δες την κοινότητα",
    emptyTitle: "Ο κατάλογος ανοίγει τώρα.", emptyBody: "Το NOXA εμφανίζει μόνο πραγματικές κοινότητες. Καθώς ελέγχονται και δημοσιεύονται, θα εμφανίζονται εδώ χωρίς placeholder profiles ή ψεύτικους αριθμούς.", emptyCta: "Καταχώρισε την κοινότητά σου", footer: "Public community discovery by NOXA.",
  },
} as const;
    ...
    <main>
      ...
      <div className={styles.shell}>
        <form className={styles.toolbar} method="get">
          ...
          <div><span className={styles.filterLabel}>Focus</span><div className={styles.filters}>
            <Link className={!selectedFocus ? styles.filterActive : styles.filter} href={focusHref(locale)}>{t.all}</Link>
            <Link className={selectedFocus === "car" ? styles.filterActive : styles.filter} href={focusHref(locale, "car")}>{t.car}</Link>
            <Link className={selectedFocus === "moto" ? styles.filterActive : styles.filter} href={focusHref(locale, "moto")}>{t.moto}</Link>
            <Link className={selectedFocus === "mixed" ? styles.filterActive : styles.filter} href={focusHref(locale, "mixed")}>{t.mixed}</Link>
          </div></div>
        </form>
        ...
        {filtered.length ? <div className={styles.grid}>{filtered.map((community) => {
          ...
          return <Link className={styles.card} href={href} key={community.id}>
            <div className={styles.cover} style={coverStyle} />
            <div className={styles.cardBody}><div className={styles.cardTop}><h2>{community.name}</h2>{community.verified ? <span className={styles.verified}>VERIFIED</span> : null}</div>
              <p className={styles.location}>{[community.city, community.region, community.country_code].filter(Boolean).join(" · ")}</p>
              {community.description ? <p className={styles.description}>{community.description}</p> : null}
              <div className={styles.tags}><span className={styles.tag}>{community.focus}</span>{community.scene_tags.slice(0, 4).map((tag) => <span className={styles.tag} key={tag}>{tag}</span>)}</div>
              <span className={styles.cardCta}>{t.view} →</span>
            </div>
          </Link>;
