...
async function publicDataFetch(path: string) {
  return fetch(`${WEBSITE_DATA_URL}${path}`, {
    headers: {
      ...
      apikey: WEBSITE_DATA_PUBLISHABLE_KEY,
    },
...
const communitySelect =
  "id,slug,name,description,city,region,country_code,focus,scene_tags,logo_url,cover_image_url,instagram_url,website_url,verified,published_at";
...
export async function loadPublishedCommunities(): Promise<PublicCommunity[]> {
  ...
  try {
    const response = await publicDataFetch(`/rest/v1/communities?${params}`);
...
export async function loadCommunityEvents(communityId: string): Promise<CommunityEvent[]> {
  ...
  try {
    const response = await publicDataFetch(`/rest/v1/radar_events?${params}`);
    if (!response.ok) return [];
...
export async function loadCommunityOrganizer(communityId: string): Promise<CommunityOrganizer | null> {
  const params = new URLSearchParams({
    select: "id,slug,name,verified,partner,partner_label",
    community_id: `eq.${communityId}`,
    status: "eq.active",
    verified: "eq.true",
    limit: "1",
  ...
  try {
    const response = await publicDataFetch(`/rest/v1/organizer_profiles?${params}`);
    if (!response.ok) return null;
    const rows = (await response.json()) as CommunityOrganizer[];
    return rows[0] ?? null;
  } catch {
    return null;
  }
