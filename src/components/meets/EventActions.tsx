...
export function EventActions({ eventId, eventTitle, eventType, startsAt, endsAt, locationLabel, organizerName, mapQuery, locale, isPast = false }: Props) {
  ...
  async function share() {
    ...
    try {
      ...
      // Native share dismissal requires no UI error.
    }
  }
  ...
  async function shareStory() {
    ...
    try {
      ...
      const gradient = ctx.createLinearGradient(0, 0, 1080, 1000);
      gradient.addColorStop(0, "#c8102e");
      gradient.addColorStop(.45, "#26080e");
      gradient.addColorStop(1, "#050505");
      ctx.fillStyle = gradient;
      ctx.fillRect(0, 0, 1080, 1080);
      ctx.fillStyle = "rgba(255,255,255,.12)";
      ctx.fillRect(72, 1120, 936, 1);
      ...
      wrapCanvasText(ctx, locationLabel, 900, 2).forEach((line, index) => ctx.fillText(line, 72, 1320 + index * 48));
      ctx.fillStyle = "#8c8e94";
      ctx.font = "600 28px -apple-system, BlinkMacSystemFont, sans-serif";
      ctx.fillText(`Organized by ${organizerName}`, 72, 1490);
      ctx.fillStyle = "#ffffff";
      ctx.font = "800 38px -apple-system, BlinkMacSystemFont, sans-serif";
      ctx.fillText("noxastreetapp.com/meets", 72, 1780);
  ...
  return <>
    ...
    <div className={styles.reportWrap}>
      ...
      {reportOpen ? <form className={styles.reportForm} onSubmit={(event) => void submitReport(event)}>
        <label><span>{labels.reason}</span><select name="reason" defaultValue="time">{labels.reasons.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>
        <label><span>{labels.details}</span><textarea name="details" maxLength={800} rows={3} required /></label>
        <label><span>{labels.email}</span><input name="email" type="email" maxLength={254} /></label>
        <button type="submit" disabled={reportState === "sending"}>{labels.send}</button>
        {reportState === "sent" ? <p>{labels.sent}</p> : null}{reportState === "error" ? <p className={styles.reportError}>{labels.error}</p> : null}
      </form> : null}
    </div>
  </>;
}
