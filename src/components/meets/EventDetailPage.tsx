...
const CATEGORY_LABELS: Record<string, string> = {
  car_meet: "CAR MEET", moto_meet: "MOTO MEET", track_day: "TRACK DAY", drag: "DRAG RACING", drift: "DRIFT", rally: "RALLY", show: "AUTO SHOW", cars_and_coffee: "CARS & COFFEE", group_drive: "GROUP DRIVE", festival: "FESTIVAL", other: "EVENT",
};
...
export async function loadPublicEvent(slug: string): Promise<EventRow | null> {
  const query = new URLSearchParams({
    select: "id,public_slug,title,event_type,starts_at,ends_at,timezone,location_text,city,region,country_code,organizer_name,organizer_url,source_name,source_url,summary,organizer_profile_id,community_id,featured,partner_badge",
    public_slug: `eq.${slug}`,
    status: "eq.published",
    limit: "1",
  });
...
function formatDate(value: string, timezone: string | null, locale: "en" | "el") {
  return new Intl.DateTimeFormat(locale === "el" ? "el-GR" : "en-GB", {
    weekday: "short", day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit", timeZone: timezone || "Europe/Athens",
...
export async function EventDetailPage({ slug, locale }: { slug: string; locale: "en" | "el" }) {
  ...
  const backHref = `${base}/meets`;
  const t = locale === "el" ? {
    past: "ΤΟ EVENT ΟΛΟΚΛΗΡΩΘΗΚΕ", featured: "FEATURED", about: "ΣΧΕΤΙΚΑ", location: "ΤΟΠΟΘΕΣΙΑ", organizer: "ΔΙΟΡΓΑΝΩΤΗΣ", official: "Επίσημη πηγή", community: "COMMUNITY", responsibility: "Οι τελικές λεπτομέρειες παραμένουν ευθύνη του organizer.", fallback: "Δες τις επίσημες πληροφορίες του organizer πριν ξεκινήσεις.",
  } : {
    past: "PAST EVENT", featured: "FEATURED", about: "ABOUT", location: "LOCATION", organizer: "ORGANIZED BY", official: "Official source", community: "COMMUNITY", responsibility: "Final event details remain the organizer's responsibility.", fallback: "Check the organizer's official details before travelling.",
  };
    ...
    <main>
      ...
      <section className={styles.content}><div className={styles.shell}><div className={styles.grid}>
        ...
        <aside className={styles.organizerCard}>
          ...
          <div className={styles.organizerBadges}>{profile?.verified ? <b>VERIFIED</b> : null}{profile?.partner ? <b>{profile.partner_label || "NOXA PARTNER"}</b> : null}</div>
          ...
          <a href={organizerUrl} rel="noreferrer" target="_blank">{t.official} ↗</a>
          {profile?.instagram_url ? <a href={profile.instagram_url} rel="noreferrer" target="_blank">Instagram ↗</a> : null}
          <small>{t.responsibility}</small>
        </aside>
      </div></div></section>
    </main>
  </div>;
