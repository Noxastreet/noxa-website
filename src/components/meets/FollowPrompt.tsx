...
export function FollowPrompt({ targetType, targetKey, targetLabel, locale, compact = false }: Props) {
  ...
  const t = locale === "el" ? {
    city: `Ακολούθησε ${targetLabel}`,
    organizer: `Ακολούθησε ${targetLabel}`,
    hint: "Αποθήκευσε το email σου για σχετικά NOXA event alerts.",
    email: "Email",
    consent: "Συμφωνώ να λαμβάνω NOXA event alerts για αυτή την επιλογή.",
    submit: "Αποθήκευση follow",
    sending: "Αποθήκευση…",
    sent: "Έτοιμο. Το follow αποθηκεύτηκε.",
    error: "Δεν αποθηκεύτηκε. Δοκίμασε ξανά.",
  } : {
    city: `Follow ${targetLabel}`,
    organizer: `Follow ${targetLabel}`,
    hint: "Save your email for relevant NOXA event alerts.",
    email: "Email",
    consent: "I agree to receive NOXA event alerts for this selection.",
    submit: "Save follow",
    sending: "Saving…",
    sent: "Done. Your follow is saved.",
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/meets/follow", {
        ...
        body: JSON.stringify({ email: form.get("email"), consent: form.get("consent") === "on", targetType, targetKey, targetLabel, locale }),
      });
