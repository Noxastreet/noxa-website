...
const CATEGORY: Record<string, string> = {
  car_meet: "CAR MEET", moto_meet: "MOTO MEET", track_day: "TRACK DAY", drag: "DRAG", drift: "DRIFT", rally: "RALLY",
  show: "AUTO SHOW", cars_and_coffee: "CARS & COFFEE", group_drive: "GROUP DRIVE", festival: "FESTIVAL", other: "EVENT",
};
...
function countryFlag(code: string) {
  return /^[A-Z]{2}$/.test(code) ? String.fromCodePoint(...code.split("").map((char) => 127397 + char.charCodeAt(0))) : "";
}
...
export function MeetsDirectory({ events, detectedCountryCode, locale }: { events: MeetsDirectoryEvent[]; detectedCountryCode: string; locale: "en" | "el" }) {
  ...
  const t = locale === "el" ? {
    eyebrow: "NOXA MEETS", title: "Βρες το επόμενο meet σου.", body: "Car meets, moto events και motorsport σε ένα μέρος.", upcoming: "Ανακάλυψε", add: "Πρόσθεσε Event",
    country: "Χώρα", city: "Πόλη", type: "Τύπος", date: "Πότε", search: "Αναζήτηση", searchPlaceholder: "Event, organizer ή πόλη", near: "Κοντά μου", locating: "Εντοπισμός…",
    all: "Όλα", car: "Cars", moto: "Moto", motorsport: "Motorsport", allCities: "Όλες οι πόλεις", noCities: "Δεν υπάρχουν πόλεις", noEvents: "Δεν υπάρχουν events με αυτά τα φίλτρα.",
    view: "Δες Event", reset: "Καθαρισμός", found: "events", now: "Τώρα", today: "Σήμερα", tomorrow: "Αύριο", weekend: "Σαββατοκύριακο", month: "Αυτόν τον μήνα", saved: "Αποθηκευμένα",
    archive: "Προηγούμενα events", live: "LIVE", featured: "FEATURED", locationNoCity: "Βρήκαμε τη χώρα σου, αλλά δεν υπάρχει ακόμη event στην πόλη σου.", locationError: "Δεν μπορέσαμε να βρούμε την τοποθεσία σου.",
  } : {
    eyebrow: "NOXA MEETS", title: "Find your next meet.", body: "Car meets, moto events and motorsport in one place.", upcoming: "Discover", add: "Add Event",
    country: "Country", city: "City", type: "Type", date: "When", search: "Search", searchPlaceholder: "Event, organizer or city", near: "Near me", locating: "Locating…",
    all: "All", car: "Cars", moto: "Moto", motorsport: "Motorsport", allCities: "All cities", noCities: "No cities yet", noEvents: "No events match these filters.",
    view: "View Event", reset: "Reset", found: "events", now: "Happening now", today: "Today", tomorrow: "Tomorrow", weekend: "This weekend", month: "This month", saved: "Saved",
  ...
  return <div className={styles.page}>
    ...
    <main>
      ...
      <section className={styles.feed}><div className={styles.shell}>
        ...
        {visible.length ? <div className={styles.grid}>{visible.map((event) => {
          const date = formatEventDate(event, locale);
          const live = isHappeningNow(event);
          return <Link className={`${styles.card} ${event.featured ? styles.featuredCard : ""}`} href={`${base}/meets/${event.slug}`} key={event.id}>
