...
function formatEventDate(event: MeetsDirectoryEvent, locale: "en" | "el") {
  ...
  return {
    ...
    time: new Intl.DateTimeFormat(formatterLocale, { hour: "2-digit", minute: "2-digit", hour12: false, timeZone }).format(start),
  };
}
...
function matchesDate(event: MeetsDirectoryEvent, filter: DateFilter, savedIds: Set<string>) {
  ...
  if (day === 0) weekendStart.setDate(weekendStart.getDate() - 1);
  else {
    ...
    <div className={styles.page}>
      ...
      <main>
        ...
        <section className={styles.feed}><div className={styles.shell}>
          ...
          {city !== "all" ? <div className={styles.cityFollow}><span>{t.followCity}</span><FollowPrompt targetType="city" targetKey={`${country}:${normalize(city)}`} targetLabel={city} locale={locale} compact /></div> : null}
          {visible.length ? <div className={styles.grid}>{visible.map((event) => { const date = formatEventDate(event, locale); const live = isHappeningNow(event); return <Link className={`${styles.card} ${event.featured ? styles.featuredCard : ""}`} href={`${locale === "el" ? "/el" : ""}/meets/${event.slug}`} key={event.id}>
