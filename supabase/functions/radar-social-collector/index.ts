...
type ExpectedAccessStatus = "rate_limited" | "unsupported";
...
function expectedAccessStatus(message: string): ExpectedAccessStatus | null {
  ...
  if (
    ...
    normalized.includes("not accessible without login")
  ) return "unsupported";
  return null;
}
...
async function fetchSocialDocument(platform: Source["platform"], rawUrl: string, sourceName: string): Promise<SocialDocument | null> {
  ...
  if ([401, 403, 429].includes(response.status)) {
    throw new Error(`Public ${platform} page is not accessible without login (${response.status})`);
  }
  if (!response.ok) throw new Error(`${platform} page returned ${response.status}`);

  const finalUrl = response.url || rawUrl;
  if (/\/login\/?/i.test(new URL(finalUrl).pathname)) {
    throw new Error(`Public ${platform} page redirected to login`);
  }
  ...
  if (/log in to (instagram|facebook)|you must log in|συνδεθειτε|συνδεση στο facebook/i.test(visible.slice(0, 1800))) {
    throw new Error(`Public ${platform} page requires login`);
  }
...
async function collectSocialSource(source: Source) {
  ...
  // Profile/page HTML is frequently hydration-driven. Also scan the raw response indirectly
  // through metadata-derived text; if no concrete public links are exposed, return no change.
  ...
  try {
    ...
    for (const source of sources) {
      ...
      try {
        ...
        const expectedStatus = expectedAccessStatus(message);
        if (expectedStatus) {
          await rest("radar_source_checks", {
            method: "POST",
            body: JSON.stringify({
