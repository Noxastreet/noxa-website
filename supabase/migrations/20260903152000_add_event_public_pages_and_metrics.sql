-- Website-only public event pages and privacy-conscious organizer metrics.
-- Production was already migrated through Supabase migration add_event_public_pages_and_metrics.
...
set public_slug = case
  when char_length(trim(both '-' from regexp_replace(lower(coalesce(title, 'event')), '[^a-z0-9]+', '-', 'g'))) >= 2
    then left(trim(both '-' from regexp_replace(lower(coalesce(title, 'event')), '[^a-z0-9]+', '-', 'g')), 72) || '-' || left(id::text, 8)
  else 'event-' || left(id::text, 8)
end
...
drop policy if exists "organizers read own event metrics" on public.event_metrics_daily;
...
begin
  ...
  if not exists (
    select 1 from public.radar_events where id = p_event_id and status = 'published'
  ) then
    return false;
  end if;
...
revoke all on function public.record_event_metric(uuid, text, text) from public, anon, authenticated;
...
comment on column public.radar_events.public_slug is
  'Stable public URL slug for website event pages.';

comment on table public.event_metrics_daily is
  'Daily aggregate website interactions for organizer-owned public events. No raw visitor identity is stored here.';

comment on table public.event_metric_rate_limits is
  'Short-lived hashed request fingerprints used only to reduce repeated metric counting.';
