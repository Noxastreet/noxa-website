-- Prevent incomplete or collector-placeholder events from becoming public.
-- Existing published events are not retroactively blocked; the gate runs on new publication/republish.
...
begin
  if new.status = 'approved' and old.status is distinct from 'approved' then
    issues := private.radar_candidate_quality_issues(new);
...
begin
  if tg_op = 'INSERT' then
    ...
    should_check := new.status = 'published' and old.status is distinct from 'published';
  end if;
...
drop trigger if exists radar_candidate_quality_gate on public.radar_candidates;
create trigger radar_candidate_quality_gate
before update of status on public.radar_candidates
for each row
execute function private.enforce_radar_candidate_quality_on_approval();

drop trigger if exists radar_event_quality_gate on public.radar_events;
create trigger radar_event_quality_gate
before insert or update of status on public.radar_events
for each row
execute function private.enforce_radar_event_quality_on_publish();

comment on function private.radar_candidate_quality_issues(public.radar_candidates) is
  'Returns machine-readable quality issues that must be fixed before a Radar candidate can be approved.';
comment on function private.radar_event_quality_issues(public.radar_events) is
  'Returns machine-readable quality issues for a public Radar event. Reviewed imports require a factual summary and a real organizer.';
