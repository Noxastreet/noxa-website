-- Reliability Gate P0: NOXA Website publishes car and moto events in Greece.
-- Raw candidates outside Greece may still be collected for audit/review, but publication must fail closed.
...
-- Existing public rows outside Greece must no longer remain public.
update public.radar_events
set status = 'unpublished', updated_at = now()
where status = 'published'
  and country_code is distinct from 'GR';

-- Leave raw candidates reviewable, but make the automatic scope decision explicit.
update public.radar_candidates
set
  auto_publish_outcome = 'blocked',
  auto_publish_reason = 'Scope Gate: outside_greece',
